                ...
                link       : function (scope, elements, attributes) {
                    ...
                    var videoProcess = {
                        ...
                        embed: function (data, options) {
                            ...
                            var t = /((http|https):\/\/|)(www\.|)twitch\.tv\/[a-zA-Z0-9_]{1,}/;
                            /**
                             * Twitch embedding
                             */

                            if (data.match(t)) {
                                scope.video.id = RegExp.$0;
                                var result = data.match(t);
                                var url = result[0];

                                var channel = url.substr(url.lastIndexOf('/') + 1)
                                console.log(channel)
                                ...
                                if (options.video.details) {
                                    $http.get('https://www.googleapis.com/youtube/v3/videos?id=' + RegExp.$1 + '&key=' + options.video.ytAuthKey + '&part=snippet,statistics')
                                        .then(function (d) {
                                             console.log(d)
                                            var ytData = d.data.items[0];

                                             scope.video.host = 'twitch';
